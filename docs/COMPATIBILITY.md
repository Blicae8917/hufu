...
| Codex | App Server 的 thread/turn 生命周期；非交互执行、resume、JSONL、JSON Schema 和 Usage 事件 | 具体权限、模型身份和取消语义仍需版本化合同测试。DeepSeek Harness 已自带 `subagent-codex`，与「Hufu 不调用真实 Desktop 工具」不是同一层事实 | #59 已交付只下发 packet 的 NativeHost RuntimeProvider；无注入适配器失败关闭；CI 不调用真实 Desktop 工具，`UNAVAILABLE` |
