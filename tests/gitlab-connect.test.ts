...
function withTempDir(fn: (dir: string) => void): void {
  const dir = mkdtempSync(join(tmpdir(), "hufu-gl-connect-"));
  try {
    fn(dir);
  } finally {
...
describe("hufu connect gitlab", () => {
  it("connects a two-segment gitlab.com project without fetching", () => {
    withTempDir((dir) => {
      const result = runHufu(GITLAB_CONNECT_ARGS, dir);
      assert.equal(result.status, 0, result.stderr);
      const payload = parseStdout(result.stdout)["result"] as Record<
        string,
        unknown
      >;
      assert.equal(payload["task_authority"], "gitlab");
      assert.equal(payload["repository_canonical"], "example-group/example-project");
      const doctor = doctorWorkspace(dir);
      assert.equal(doctor.task_authority, "gitlab");
      assert.equal(doctor.healthy, true);
    });
  });

  it("rejects nested groups and GitHub URLs with exit 2", () => {
    withTempDir((dir) => {
      const nested = GITLAB_CONNECT_ARGS.map((value) =>
        value === "example-group/example-project" ? "group/sub/project" : value,
      );
      const nestedResult = runHufu(nested, dir);
      assert.equal(nestedResult.status, 2);
      assert.equal(existsSync(join(dir, ".hufu")), false);

      const github = GITLAB_CONNECT_ARGS.map((value) =>
        value === "example-group/example-project"
          ? "https://github.com/Blicae8917/hufu"
          : value,
      );
      const githubResult = runHufu(github, dir);
      assert.equal(githubResult.status, 2);
      const error = parseStdout(githubResult.stdout)["error"] as Record<
        string,
        unknown
      >;
  ...
  it("rejects gitlab after a local or github connection", () => {
    withTempDir((dir) => {
      const local = runHufu(
        [
          "connect",
  ...
  it("rejects opening a local work item on a gitlab project", () => {
    withTempDir((dir) => {
