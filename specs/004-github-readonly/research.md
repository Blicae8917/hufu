...
- **Decision**: 网络错误、非 2xx、无 JSON、配额耗尽 → 刷新命令失败（退出码 4，`OBSERVATION_UNAVAILABLE`），保留旧缓存；随后无刷新的 `status` 仍可成功，工作项时效为 `stale` 或 `unavailable`（无缓存则为数据不足）。`observed_at` 早于 `stale_after_hours`（默认 24）→ `stale`。GitHub 未给的数字字段不得写 `0`。
