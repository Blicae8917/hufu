...
- 不得在成功路径访问网络或监听端口。
- 工作目录为当前进程 cwd；账本位于 `cwd/.hufu/ledger/`。

## `hufu connect`

```text
hufu connect --project-id <id> --repository <uri> --task-authority local --commander <id> --grant-scope <text> [--project-lead <id>] [--grant-expires <iso8601>]
```
...
```text
hufu doctor [--repair-truncated-tail]
```
